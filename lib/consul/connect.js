...
const parse = require('@sealsystems/seal-parse-duration');
...
const connect = async function (options) {
  ...
  let adr = consulAddress.address;
  if (options.dnsPort) {
    // set different port for DNS, e.g. for using consul service directly on port 8600
    adr = `${adr}:${options.dnsPort}`;
  }
  dns.setServers([adr]);
