...
const watchEnv = async function (paths) {
  ...
  for (const path of paths) {
    ...
    watch.on('change', () => {
      if (watch.listenOnChange) {
        watcher.emit('error', new Error('Configuration changed, reloading...'));
      }
    ...
    watch.on('error', (err) => {
      ...
      watcher.emit('error', new Error('Consul error'));
    });
...
const setEnv = async function (options) {
  ...
  return await watchEnv.call(this, paths);
};
